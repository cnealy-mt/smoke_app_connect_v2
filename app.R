...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    conditionalPanel(
      ...
      p(""),
      h4("Outlooks"),
      p("Use the 'variable' selector above to switch between map outlook views and the 'lead time' selector to choose the forecast day (listed in the top right of the map). Use the selector below the map to view 48-hour meteorological timeseries for each Montana county."),
      
      h4("AQI Outlooks"),
      p("View average and maximum (1-hour) AQI values for the selected forecast day. Average AQI can be viewed by county average or area average and is calculated from expected near-surface PM2.5 concentrations over the forecast day hours.", style = "margin-left: 20px;"),
      p("For the HRRR model, the 48 hour forecast initializes at 6am MST, so fewer hours are present for the first day (lead time 0).", style = "margin-left: 20px;"),
      img(src = "AQI_scale.png", width = "60%", alt = "AQI Color Scale", style = "margin-left: 20px;"),
      
      h4("Ventilation Rate"),
      p("The ventilation rate view shades counties by their daily maximum (1-hour) ventilation rate. Breaks are based on those from the University of Washington.", style = "margin-left: 20px;"),
      p("Select the ventilation window variable and another dropdown menu will appear. With these options, you can view the number of hours that will be equal to or above either the 'good' or 'marginal' ventilation threshold.", style = "margin-left: 20px;"),
      ...
      p("All HRRR model data in this app is scheduled to be updated at 8:05am, but may take longer to process. If the forecast date on the map matches today's date for lead time 0, then the model data has been successfully updated.", style = "font-size: 90%; font-style: italic;")
    ),

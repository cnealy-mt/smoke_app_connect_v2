...
CWFIS_url <- paste0("https://geoserver.cwfif.nrcan.gc.ca/geoserver/wfs?service=WFS&version=2.0.0&request=GetFeature&outputFormat=csv&typeName=public:cwfif_national_activefires&CQL_FILTER=record_start%3E=", 
                    current_year, "-01-01T00:00:00z%20AND%20record_end%3E=", 
                    current_year, "-12-31T23:59:59z&_gl=1*5vhdh0*_ga*MTg5OTgxNDAyMC4xNzc1MTQxNDAy*_ga_C2N57Y7DX5*czE3NzUxNjExMTYkbzMkZzEkdDE3NzUxNjExMjMkajUzJGwwJGgw")

# 1. Read CAN fire data, clean and rename columns
CAN_point_data <- read.csv(CWFIS_url) %>%
  mutate(IncidentSize = round(fire_size * 2.47105, 0),  # hectares to acres approx.
         IncidentTypeCategory = ifelse(fire_was_prescribed == 1, "RX", "WF"),
         PercentContained = if_else(percent_contained == -1, NA_real_, as.numeric(percent_contained))) %>%
  rename(UniqueFireIdentifier = national_fire_id, 
         x = longitude, 
         y = latitude, 
         FireDiscoveryDateTime = situation_report_date, 
         PointModDateTime = status_date) %>%
  select(UniqueFireIdentifier, FireDiscoveryDateTime, PointModDateTime, IncidentTypeCategory, PercentContained, IncidentSize, x, y) %>%
  mutate(FireDiscoveryDateTime = ymd_hms(FireDiscoveryDateTime),
         PointModDateTime = ymd_hms(PointModDateTime)) %>%
  mutate(FireDiscoveryDateTime = format(FireDiscoveryDateTime, "%Y-%m-%d %H:%M:%S"),
         PointModDateTime = format(PointModDateTime, "%Y-%m-%d %H:%M:%S"))
...
convert_to_sf <- function(perimeters) {
  ...
  polygons <- lapply(perimeters, function(ring) {
    ...
    # --- SAFETY GATE: Skip if the matrix does not have exactly 2 columns (X, Y) or lacks points ---
    if (ncol(ring) != 2 || nrow(ring) < 3) {
      return(NULL)
    }
  ...
  # Remove any NULL entries caused by corrupted rings using Base R
  polygons <- polygons[!sapply(polygons, is.null)] 
  
  # Fallback if the entire fire perimeter's ring data was corrupt

...
if (file.exists(airnow_path)) {
  ...
  if (exists("GEOS_pm25_stack") && !is.null(GEOS_pm25_stack)) {
    cat("✅ GEOS_pm25_stack exists in memory and is not NULL\n")
    
    # Extract values
    values <- terra::extract(GEOS_pm25_stack, site_vect)
    ...
    reshape_smoke_data <- function(values) {
      ...
      smoke_long <- values %>%
        ...
        mutate(
          ...
          fcst_hour = as.integer(sub("pm25_", "", fcst_hour)) * 3 - 3,

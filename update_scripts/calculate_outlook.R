...
for (type in names(all_vars)) {
  for (var in all_vars[[type]]) {
    ...
    # 1. Math in UTC: 1st layer is f00 (index 1 - 1 = 0 hours)
    layer_datetime_utc <- model_runtime + hours(layer_index) - hours(1)
    
    # 2. Convert to Denver (Handles the 2AM skip correctly)
